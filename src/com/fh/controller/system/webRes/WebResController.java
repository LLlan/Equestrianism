package com.fh.controller.system.webRes;
...
@Controller
@RequestMapping(value="/webRes")
public class WebResController extends BaseController{
	...
	public ModelAndView getlistPage(Page page) throws Exception{
		...
		pd = this.getPageData();
		page.setPd(pd);
		List<PageData> list=webResService.getlistPage(page);
		mv.addObject("pd", pd);
		mv.addObject("list",list);
		mv.setViewName("system/webRes/webRes_list");
		return mv;
	}
	/**
	 * 跳转到添加信息页
	...
	public ModelAndView toAdd(){
		...
		mv.setViewName("system/webRes/webRes_edit");
		return mv;
	}
	/**
	 * 验证是否存在
	...
	public Object selectByName() throws Exception{
		...
		pd1=webResService.getDataByName(pd);
	...
	public ModelAndView insert() throws Exception{
		...
		PageData pd=new PageData();
		pd=this.getPageData();
		pd.put("website_id", this.get32UUID());
		webResService.insert(pd);
	...
	public ModelAndView toEdit() throws Exception{
		...
		PageData pd=new PageData();
		pd=this.getPageData();
		pd.put("website_id", pd.get("tagID"));
		pd=webResService.getDataById(pd);
	...
	public void delete(PrintWriter writer) throws Exception{
		...
		pd=this.getPageData();
		pd.put("website_id", pd.get("tagID"));
		webResService.delete(pd);
		webResService.delete1(pd);
		writer.close();
	}
	/**
	 * 批量删除记录
	...
	public Object deleteAll(){
		...
		try {
			...
			if(USER_ids!=null && !USER_ids.equals("")){
				...
				webResService.deleteAll(ArrayUSER_ids);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
