package com.fh.controller.system.rukouType;
...
@Controller
@RequestMapping(value="/rukouType")
public class RukouTypeController extends BaseController{
	...
	public ModelAndView getlistPage(Page page) throws Exception{
		...
		pd = this.getPageData();
		page.setPd(pd);
		List<PageData> list=rukouTypeService.getlistPage(page);
		mv.addObject("pd", pd);
		mv.addObject("list",list);
		mv.setViewName("system/rukouType/rukouType_list");
		return mv;
	}
	/**
	 * 跳转到添加信息页
	...
	public ModelAndView toAdd(){
		...
		mv.setViewName("system/rukouType/rukouType_edit");
		return mv;
	}
	/**
	 * 验证是否存在
	...
	public Object selectByName() throws Exception{
		...
		pd1=rukouTypeService.getDataByName(pd);
	...
	public ModelAndView insert() throws Exception{
		...
		PageData pd=new PageData();
		pd=this.getPageData();
		pd.put("rukouType_id", this.get32UUID());
		rukouTypeService.insert(pd);
	...
	public ModelAndView toEdit() throws Exception{
		...
		PageData pd=new PageData();
		pd=this.getPageData();
		pd.put("rukouType_id", pd.get("tagID"));
		pd=rukouTypeService.getDataById(pd);
	...
	public Object deleteAll(){
		...
		try {
			...
			if(USER_ids!=null && !USER_ids.equals("")){
				...
				rukouTypeService.deleteAll(ArrayUSER_ids);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
