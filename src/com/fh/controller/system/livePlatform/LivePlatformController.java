package com.fh.controller.system.livePlatform;
...
@Controller
@RequestMapping(value="/livePlatform")
public class LivePlatformController extends BaseController{
	...
	public ModelAndView getlistPage(Page page) throws Exception{
		...
		pd = this.getPageData();
		page.setPd(pd);
		List<PageData> list=livePlatformService.getlistPage(page);
		mv.addObject("pd", pd);
		mv.addObject("list",list);
		mv.setViewName("system/livePlatform/livePlatform_list");
		return mv;
	}
	/**
	 * 跳转到添加信息页
	...
	public ModelAndView toAdd(){
		...
		mv.setViewName("system/livePlatform/livePlatform_edit");
		return mv;
	}
	/**
	 * 验证是否存在
	...
	public Object selectByName() throws Exception{
		...
		pd1=livePlatformService.getDataByName(pd);
	...
	public ModelAndView toEdit() throws Exception{
		...
		pd=livePlatformService.getDataById(pd);
	...
	public Object deleteAll(){
		...
		try {
			...
			if(USER_ids!=null && !USER_ids.equals("")){
				...
				livePlatformService.deleteAll(ArrayUSER_ids);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
