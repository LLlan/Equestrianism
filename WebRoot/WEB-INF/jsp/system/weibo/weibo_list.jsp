...
<script type="text/javascript">
	...
	function checkedNo(obj,tagID){
		 var url='api/xmtv1/sysWbChecked?tagID='+tagID+'&num=1';
	...
	function checkedYes(obj,tagID){
		 var url='api/xmtv1/sysWbChecked?tagID='+tagID+'&num=2';
		 $.get(url,function(data){
			 $("#"+tagID).html("已通过");
		 });
	}
			...
			bootbox.confirm(msg,function(result){
				if(result){
					...
					if(str==''){
						...
						$.ajax({
							type:'post',
							url:'<%=basePath%>api/xmtv1/sysWbChecked.do',
							dataTyoe:'json',
							cache: false,
							data:{
								"ids":str,
...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="<%=basePath%>/api/xmtv1/sysWblistPage.do" method="post" name="userForm" id="userForm">
	<table>
		<tr>
			<td>
			...
			<c:when test="${not empty list}">
				...
				<tr>
					<td class='center' style="width: 30px;">
				 		<label><input type='checkbox' name='ids' value="${var.microBlog_id }"/><span class="lbl"></span></label>
					</td>
					<td class='center' style="width: 30px;">${vs.index+1}</td>
					<td class='center'>
						<a href="${var.microBlogHttp}" target="_blank">点击进入微博</a>
					</td>
					<td class='center'>${var.name}</td>
					<td class='center'>${var.fansNum}</td>
					<td class='center'>${var.province}/${var.city}</td>
					<td class='center'>
						<img alt="" src="<%=basePath%>${var.headImgURL}" style="width: 30px;height: 30px;">
					</td>
					<td class='center' id="${var.microBlog_id }">
					...
					<td style="width: 80px;">
						<a class='btn btn-mini btn-info' title="拒绝" onclick="checkedNo(this,'${var.microBlog_id }')" >拒绝</a>
						<a class='btn btn-mini btn-danger' title="通过"  onclick="checkedYes(this,'${var.microBlog_id }')">通过</a>
					</td>
				</tr>
				</c:forEach>
			</c:when>
