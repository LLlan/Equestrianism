...
<script type="text/javascript">
	...
	function add(PARENT_ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>webChannel/toAdd.do';
		 diag.Width = 223;
		 diag.Height = 175;
	...
	function edit(tagID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>webChannel/toEdit.do?tagID='+tagID;
		 diag.Width = 223;
		 diag.Height = 175;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	//删除
		function del(tagID){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>webChannel/delete.do?tagID='+tagID;
			...
			bootbox.confirm('确定执行批量删除操作吗？',function(result){
				if(result){
					...
					if(str==''){
						...
						$.ajax({
							type:'post',
							url:'<%=basePath%>webChannel/deleteAll.do',
							dataTyoe:'json',
							cache: false,
							data:{
								"USER_ids":str
...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="<%=basePath%>/webChannel/getlistPage.do" method="post" name="userForm" id="userForm">
	<table>
		<tr>
			<td>
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<th class='center'>频道名称</th>
			<th class='center'>所属网站</th>
