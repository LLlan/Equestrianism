...
<script type="text/javascript">
	...
	function checkedNo(obj,tagID){
		 var url='api/xmtv1/sysGzhChecked?tagID='+tagID+'&num=1';
	...
	function checkedYes(obj,tagID){
		 var url='api/xmtv1/sysGzhChecked?tagID='+tagID+'&num=2';
		 $.get(url,function(data){
			 $("#"+tagID).html("已通过");
		 });
	}
			...
			bootbox.confirm(msg,function(result){
				if(result){
					...
					if(str==''){
						...
						$.ajax({
							type:'post',
							url:'<%=basePath%>api/xmtv1/sysGzhChecked.do',
							dataTyoe:'json',
							cache: false,
							data:{
								"ids":str,
			...
			<c:when test="${not empty list}">
				...
				<tr>
					<td class='center' style="width: 30px;">
				 		<label><input type='checkbox' name='ids' value="${var.WCpublic_id }"/><span class="lbl"></span></label>
					</td>
					...
					<td style="width: 80px;">
						<a class='btn btn-mini btn-info' title="拒绝" onclick="checkedNo(this,'${var.WCpublic_id }')" >拒绝</a>
						<a class='btn btn-mini btn-danger' title="通过"  onclick="checkedYes(this,'${var.WCpublic_id }')">通过</a>
					</td>
				</tr>
				</c:forEach>
			</c:when>
