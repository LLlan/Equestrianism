...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>rukouType/toAdd.do';
		 diag.Width = 223;
		 diag.Height = 175;
	...
	function edit(tagID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>rukouType/toEdit.do?tagID='+tagID;
		 diag.Width = 223;
		 diag.Height = 175;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	//删除
		function del(tagID){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>rukouType/delete.do?tagID='+tagID;
			...
			bootbox.confirm('确定执行批量删除操作吗？',function(result){
				if(result){
					...
					if(str==''){
						...
						$.ajax({
							type:'post',
							url:'<%=basePath%>rukouType/deleteAll.do',
							dataTyoe:'json',
							cache: false,
							data:{
								"USER_ids":str
...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="<%=basePath%>/rukouType/getlistPage.do" method="post" name="userForm" id="userForm">
			...
			<c:when test="${not empty list}">
				...
				<tr>
					<td class='center' style="width: 30px;">
				 		<label><input type='checkbox' name='ids' value="${var.rukouType_id }"/><span class="lbl"></span></label>
					...
					<td class='center'>${var.rukouType}</td>
					<td style="width: 68px;">
						<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${var.rukouType_id }')" ><i class='icon-edit'></i></a>
						<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${var.rukouType_id }')"><i class='icon-trash'></i></a>
					</td>
				</tr>
				</c:forEach>
			</c:when>
