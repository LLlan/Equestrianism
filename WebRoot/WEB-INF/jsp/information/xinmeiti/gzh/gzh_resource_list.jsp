...
<base href="<%=basePath%>">
    ...
    <link rel="stylesheet" href="static/css/xinmeiti/gzh_resource_list.css"/>
    <script src="static/js/xinmeiti/jquery-2.1.1.min.js"></script>
    <script src="static/js/xinmeiti/page.js"></script>
    <script src="static/js/xinmeiti/gzh_resource_list.js"></script>
...
<div class="Resources">
    <ul>
        <li><a href="javascript:void(0)" style="color: #ffcc28;" onclick="liuSourcejump(1)">微信公众号</a></li>
    ...
    <div class="container_middle">
        ...
        <script type="text/javascript">
        	...
        	function addgzhsource(){
        		var url='api/xmtv1/liuAddSourcejump.do?num=1';
        		window.location.href=url;
        	}
        </script>
        <table class="table">
            ...
            <tr>
                ...
                <th>多图文第一条(硬广/软广)</th>
                <th>多图文第二条(硬广/软广)</th>
                <th>多图文第3~N条(硬广/软广)</th>
                <th>资源状态</th>
                <th>审核状态</th>
                <th>操作</th>
            </tr>
            <c:forEach items="${list }" var="list" varStatus="vs" end="9">
            	<tr>
            	<td class='center' style="width: 30px;">
            		<c:if test="${list.checkedState=='1' }">
            			<label><input type='checkbox' name='ids' value="${list.WCpublic_id }"/><span class="lbl"></span></label>
            		</c:if>
				</td>
                ...
                <td>${list.name}</td>
                <td>${list.fansNum}</td>
                <td>￥${list.price1}/${list.price2}</td>
                <td>￥${list.price3}/${list.price4}</td>
                <td>￥${list.price5}/${list.price6}</td>
                <td>￥${list.price7}/${list.price8}</td>
                ...
                <td>
                    <a href="javascript:void(0)" target="_blank">
                        <span class="btn_b" onclick="gzhtoUpdate('${list.WCpublic_id }')" >编辑</span>
                    </a>
                </td>
            </tr>
            </c:forEach>
...
<script>
    ...
    //点击编辑
    function gzhtoUpdate(tag_id) {
    	//var newpageUrl = window.location.href;
		var url='api/xmtv1/toGzhEdite.do?tag_id='+tag_id;
		window.location.href=url;
	}
  //批量启用和下架
	function changeStateAll(msg) {
		...
		//bootbox.confirm(msg,function(result){
			//if(result){
				...
				$.ajax({
					type:'post',
					url:'api/xmtv1/changeGzhsourceState.do',
					dataTyoe:'json',
					cache: false,
					data:{
						"ids":str,
