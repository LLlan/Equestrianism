...
<div class="Resources">
    <ul>
        ...
        <li><a href="javascript:void(0)" onclick="liuSourcejump(6)" style="color: #ffcc28;">黄金广告商</a></li>
    </ul>
</div>
...
<form action="api/guangGaoOrderList/guangGaoselectlistpage.do" method="post" id="f">
<!--主体-->
<!--派单列表-->
<div class="body">
    <div class="container">
        ...
        <div class="my_Timer clear" style="padding: 20px 0 40px 0   ">
            <div class="w_water">
                订单名称
                <input type="text" class="run_name run_water" id="newsOrder_title" name="arimOrder_title" >
            </div>
            <div class="w_water" style="margin-left: 40px;">
                订单编号
                <input type="text" class="run_name  run_water" id="newsOrder_id" name="arimOrder_id">
            </div>
            <div>
                <button class="btn_more btn btn-warning btn_alter " id="sosuo" style="float: left">搜索</button>
            </div>
        ...
        <table style="display: block"  class="table">
            ...
            <tr>
            	<th>${pd.arimOrder_id }</th>
            	<th>${pd.arimOrder_title }</th>
            	<th>${pd.media_name }</th>
            	<th>${pd.price }</th>
            	<th>${pd.newsOrder_optime }</th>
            	<c:if test="${pd.order_state =='0' }">
            	<th>未通过</th>
            	</c:if>
            	<c:if test="${pd.order_state =='1' }">
            	<th>未开始</th>
            	</c:if>
            	<c:if test="${pd.order_state =='2' }">
            	<th>进行中</th>
            	</c:if>
            	<c:if test="${pd.order_state =='3' }">
            	<th>待确认</th>
            	</c:if>
            	<c:if test="${pd.order_state =='4' }">
            	<th>已完成</th>
            	</c:if>
            </tr>
            </c:forEach>
