$(function(){
    ...
    /*选择网站*/
    $(".available .col-xs-3 input").click(function(){
        $(".water input:first-child").click(function(){
            $(".modal").hide();
            $(".modal-backdrop").remove();
            $("#media").val($(this).parent().prev().find(".available .col-xs-3 input:checked").next("label").text())
            $("body").css("overflow","scroll");
        })
    });
    /*选择频道*/
    $(".bottled .col-xs-3 input").click(function(){
        $(".collection input:first-child").click(function(){
            $(".modal").hide();
            $(".modal-backdrop").remove();
            $("#channelName").val($(this).parent().prev().find(".bottled .col-xs-3 input:checked").next("label").text())
            $("body").css("overflow","scroll");
        })
    });
    ...
    $(".website .col-xs-3 input").click(function(){
        $(".first input:first-child").click(function(){
            $(".modal").hide();
            $(".modal-backdrop").remove();
            $("#media_name").val($(this).parent().prev().find(".website .col-xs-3 input:checked").next("label").text())
            $("body").css("overflow","scroll");
        })
    });
    /*ѡ��Ƶ��*/
  ...
  //入口链接
    ...
    //文件大小
    	$("#file_size").blur(function(){
        	...
        	if($("#file_size").val()!='' && !znumReg.test($("#file_size").val())){
        		$("#file_size").tips({
    				side : 2,
    				msg : '请输入正确的数字',
    				bg : '#FF0080',
    ...
    //文字字数
    	$("#word_size").blur(function(){
        	...
        	if($("#word_size").val()!='' && !znumReg.test($("#word_size").val())){
        		$("#word_size").tips({
    				side : 2,
    				msg : '请输入正确的数字',
    				bg : '#FF0080',
  ...
  //报价
    ...
    $("#buttona").click(function(){
		...
		if($("#file_size").val() != '' && !numReg.test($("#file_size").val())){
			$("#file_size").tips({
				side : 2,
				msg : '请输数字',
				bg : '#FF0080',
		...
		if($("#word_size").val() != '' && !numReg.test($("#word_size").val())){
			$("#word_size").tips({
				side : 2,
				msg : '请输数字',
				bg : '#FF0080',
		...
		if($("#caozuoType").val()=='insert'){
    		if($("#fileadv").val()=='' || !((/\.(gif|jpg|jpeg|png|bmp)$/i).test($("#fileadv").val()))){
        		$("#fileadv").tips({
    				side : 2,
		...
		$("#advForm").prepend('<input type="hidden" name="names" value="'+names+'"><input type="hidden" name="prices" value="'+prices+'">');
		//所有条件满足后，提交form表单
		$("#advForm").submit();
